...
| TASK-008 | Batch collector: snapshot + metrics (steps 3-5) | 2026-07-08 | Data/AI Implementer | `data-ai/TASK-008-snapshot-metrics`. `app/core/snapshot_metrics.py` implements Technical Design §6 steps 3-5 only (no signal detection, collection logs, or AI trigger). Verified end-to-end against the real 50-record `normalized_samples.json` from `TASK-007` via an in-memory SQLite local/dev-safe path (`tests/test_snapshot_metrics.py`, 18 tests) - no shared/production database was written to, per the AGENTS.md approval gate. New markets correctly fall back to `insufficient_data` instead of a computed delta; `market_snapshots`/`market_metrics` batch inserts retry once then log-and-skip a failing market without crashing the run. See ADR-015 for two scope judgment calls: bootstrapping `markets`/`market_outcomes` rows (necessary FK plumbing TASK-007 never does) and shipping only the P0 metric subset (`change_24h`, `change_7d`, a placeholder `heat_score`, and `sufficient`/`insufficient_data` confidence levels only - `caution_low_activity`/`caution_high_volatility`/`volatility_score`/`attention_score` deferred, tracked as TD-008). Backend Implementer should re-verify against a real Postgres once `TASK-002`'s migration is approved for a shared/dev database. |
