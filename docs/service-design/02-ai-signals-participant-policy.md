...
The v8 briefing writer produces flexible issue-centered prose only inside a
typed evidence envelope and deterministic safety validation. The separate
scenario writer may explore conditional paths but cannot assert real outcomes,
promote assumptions to facts, or access external tools.
...
V1-v7 shapes and their supersession records are historical. Accepted decisions
remain in `memory/decisions.md`, and detailed artifacts remain recoverable from
Git history.

### 6.12 V8 issue-centered narrative contract

The writer organizes authored content around the questions a general reader
asks: what the issue is, what is
...
observed change. Superseded contracts remain available from Git history.

### 6.13 V8 source retrieval

Research widens discovery before the v8 publication blockers run. It uses a
deterministic 90-day or 180-day issue horizon,
...
### 6.14 Active-v8 contextual wording policy
...
### 6.15 Validated-block streaming contract
...
### 6.16 Current summary and scenario policy

The product separates the stable v8 issue briefing from an issue-scoped
scenario conversation. They use independent request, storage, validation, and
presentation paths.
...
The scenario implementation is issue-scoped, anonymous, short-lived,
tool-free, and unable to browse, query the database, execute code, open user
URLs, or perform external actions. Application flags default to disabled; the
checked-in production Compose profile enables the worker and scenario API
explicitly without changing the v8 briefing contract.
...
| Context-candidate timing overlay | A change episode and verified context occupy a compatible review window                 | Stored episode, verified context, citation sources | N/A — display only, not a computed trigger                                   | —                                    | Highest causal-misread risk of any element in the product                                                                               | Always shown with an explicit no-relationship boundary       | **Current verified-only path**                                                                              |
...
| Historical behavior of public addresses                                  | Same data is technically public on-chain, but publishing it in _this_ product reframes it as "worth watching" | High       | High                                                                 | Medium–High        | Feasible                                                                          | **Limit** heavily; do not build a search/lookup feature for any address                                                                                                      |
