...
| TD-005 | `backend/API_CONTRACT.md`'s Error shape section documented invalid `window`/`sort` as `400`, but the actual (unchanged) FastAPI `Query(pattern=...)` validation returns `422` with a `{"detail": [...]}` list shape. | Low; documentation-only mismatch, does not affect implemented behavior. Frontend should code against `422`, not `400`. | Documented as the actual behavior in `API_CONTRACT.md` during `TASK-010`; fixing the code to force `400` instead would be a broader behavior change than "read path only" and needs separate approval if desired. |
