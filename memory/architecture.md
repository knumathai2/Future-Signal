...
- `TASK-054` replaces the dashboard-v1 state flow with real browser routes. Home is a five-issue discovery surface; `/issues` provides URL-backed client search/filter/sort/pagination over the existing summary API; detail core/history/report reads are independently cancellable; every route has a main landmark, shared navigation, and direct-entry handling. Scoped Vercel SPA rewrites do not intercept `/api`.
