...
6. Day 4 active: qualifying markets (new expectation-shift row, no report yet, or stale >24h) get a new `ai_reports` row via template-constrained generation plus a safety filter; `TASK-041` closes the remaining readiness gap for historical-seed metric timestamps
...
- `TASK-041` is active: the configured development DB currently has `ai_reports=0`. Latest historical-seed metric rows use `computed_at = latest_snapshot.captured_at + 1 microsecond`, while `build_prompt_inputs_for_market()` currently requires exact timestamp equality. The expected fix is to select the latest snapshot at or before the metric run timestamp without fabricating values, then verify success-row insertion with a fake `LLMClient`.
...
- Day 4 is active from this baseline with `TASK-041`, `TASK-040`, and `TASK-018` remaining. Any shared or production schema application, paid external AI call, public API shape change, deployment, or infrastructure change remains separately approval-gated.
