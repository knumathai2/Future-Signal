...
- Next backend step after schema application is separately approved and available: wire `app/api/routes/issues.py`/`categories.py` to `app.db.session.get_db()` and remove the hardcoded sample data (this is `TASK-010`, currently in `tasks/backlog.md`).
