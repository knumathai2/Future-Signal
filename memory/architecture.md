...
7. FastAPI serves the available read-only data; `TASK-039` wires the report endpoint to stored summaries while preserving the accepted empty state, `TASK-043` introduces the v2 schema, and `ISS-007` makes report reads require the current prompt version so legacy rows are retained but not served
...
- `/api/categories` is a read-only Korean display taxonomy derived from currently servable live issue titles/categories when DB data exists; DB-free/failure mode keeps the documented Korean sample fallback. `/api/issues?category=...` accepts those Korean labels and raw stored category values.
...
- `GET /api/issues`, `/api/issues/:id`, and `/api/issues/:id/history` read latest available snapshot/metric/history rows when a database is configured, and otherwise serve the documented static fallback with honest `data_as_of` timestamps (ADR-013). `/api/issues` category filters use the Korean taxonomy in `app/core/category_taxonomy.py`, including topic groups such as `우크라이나 전쟁` and future `이란 전쟁` matching. `/api/issues/:id/history` returns empty `points` when history is unavailable rather than fabricating a latest-point chart. `/api/issues/:id/report` reads the latest successful current-prompt-version `ai_reports` row in live mode and preserves the accepted no-report-yet response when no current successful report exists or the report read fails.
