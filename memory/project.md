...
| Data/AI | Existing v3 generation remains intact. TASK-058 adds a DB-free, bounded OpenRouter server-tool research client that trusts only API citation annotations, maps candidate URLs by exact match, rejects model-body URLs, and captures secret-free usage for TASK-059 verification. |
