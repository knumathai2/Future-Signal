...
- **Overall health**: 🟢 Good — Day 4 core UI/API/event-candidate work, report-generation code readiness, Korean live category filtering, and the 24h combined batch path are complete; OpenRouter-backed v2 summaries have been generated and verified for the default top-20 heat-sorted issues
...
| 2026-07-09 | Category filters were localized through ADR-031: `/api/categories` now returns Korean display groups such as `우크라이나 전쟁`, `이스라엘·가자`, `국제 안보`, `미국 정치`, `가상자산`, and `AI·기술`; future Iran-related issues are mapped to `이란 전쟁`. |
